...
data_dir = 'path_to/' ## path to folder where splitted to test & train 
Dataset_names_path = "path_to/classes.txt"  ## create a txt for getting the class names
Dataset_train = "path_to/train_yolov4.txt" ##creating a txt for storing train annotations
Dataset_test = "path_to/test_yolov4.txt" ##creating a txt for storing train annotations
is_subfolder = False
...
def run_XML_to_YOLOv3():
    for i, folder in enumerate(['train/annotations','test/annotations']):
    ...
    with open(Dataset_names_path, "a+") as file:
